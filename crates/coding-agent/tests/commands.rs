...
impl FauxHubJoinServer {
    async fn start() -> Self {
        ...
        let app = axum::Router::new()
            .route("/auth/start", axum::routing::post(faux_hub_join_auth_start))
            .route(
                "/auth/exchange_code",
                axum::routing::post(faux_hub_join_auth_exchange),
            )
...
async fn faux_hub_join_auth_start(
    axum::extract::State(state): axum::extract::State<Arc<tokio::sync::Mutex<FauxHubJoinState>>>,
...
#[tokio::test]
async fn dispatch_hub_join_success_outputs_safe_user_text_and_stores_credential() {
    let _auth_guard = auth::ENV_LOCK.lock().unwrap();
    let _pie_guard = PIE_DIR_ENV_LOCK.lock().unwrap();
    let _output_guard = COMMAND_OUTPUT_LOCK.lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
